        ...
        else
        {
            for (int i = 0; i < vertCount; ++i)
            {
                ...
                if (emojiDic.TryGetValue(index, out info))
                {
                    ...
                    //计算2个XX的距离
                    float emojiSize = 2 * (verts[t + 1].position.x - verts[t].position.x) * ICON_SCALE_OF_DOUBLE_SYMBOLE;

                    float fCharHeight = verts[t + 1].position.y - verts[t + 2].position.y;
                    float fCharWidth = verts[t + 1].position.x - verts[t].position.x;

                    float fHeightOffsetHalf = (emojiSize - fCharHeight) * 0.5f;
                    float fStartOffset = emojiSize * (1 - ICON_SCALE_OF_DOUBLE_SYMBOLE);

                    m_TempVerts[3] = verts[t];//1
                    m_TempVerts[2] = verts[t + 1];//2
